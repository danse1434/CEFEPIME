...
##  Jamsen KM, Patel K, Nieforth K, et al. A Regression Approach to Visual 
##  Predictive Checks for Population Pharmacometric Models. CPT Pharmacometrics 
##  Syst Pharmacol 2018; 7: 678–686.
##  
...
##########################################################################-
# Funciones para cálculo de VPC loess -------------------------------------
##########################################################################-
# Función aicc.loess
# Se calcula AIC_C para un ajuste de regresión LOESS de Hurvich, C.M., 
# Simonoff, J.S., and Tsai, C. L. 1998. Smoothing parameter selection in 
...
##:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
##  1 Verificar que el argumento fit corresponde a un objeto de tipo LOESS
##  2 Calcular n como el número de observaciones para la construcción del 
##  modelo.
##  3 Calcular la traza de la matriz hat del modelo (hyy^Th^T)
##  4 Calcular la varianza de los residuales
##  5 Calcular AIC de acuerdo a la ecuación propuesta
##:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
...
##########################################################################-
# Función autoloess
# Calcular el ajuste de loess que tiene una minimización en el espacio AIC_C
# Span = un vector de dos valores que representa los valores de span mínimo 
# y máximo.
# Retorna ajuste LOESS con span minimizando la función AIC_C
##:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
##  1 Verificar que fit es un objeto de tipo LOESS
##  2 Crear una función f que toma como argumento el span, que calcula el 
##  AIC de acuerdo a la función *aicc.loess*. Se recalcula de acuerdo al 
##  nuevo span.
##  3 Actualizar el modelo de acuerdo al span, que es optimizado para encontrar 
##  el valor óptimo. Se obtiene un modelo *fit* con la mejor optimización.
##  4 Devolver opt.span que tiene el span mínimo en el espacio muestral.
##:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
...
out1  <- list(name = 'y_1', time = seq(0, 8, length.out = 5e2))

data_list <- vector(mode = "list", length = 5e2)
...
PRED_Expect <-
  loess(y_1 ~ time, data = data_pred, span = 0.3) %>% 
  autoloess(.)

data_pred = data_pred %>% 
  mutate(PRED_E = predict(PRED_Expect[['fit']])) %>% 
  mutate(time = round(time, 4)) %>% 
  rename(PRED = y_1)

simul_dfr <- data_list %>%
  map(~ mutate(.x, time = round(time, 4))) %>%
  map(~ left_join(.x, data_pred, by = c('id', 'time'))) %>%
  map( ~ mutate(.x, pc = y_1 * PRED_E / PRED)) %>%
  map_dfr(~ as.data.frame(.x), .id = 'sim_id')


AQRREG <- function(df, tau) {
  AIC_calc <- function(data = df, llam, tau) {
    a = data %>%
      rqss(
        pc ~ qss(time, lambda = exp(llam)),
        data = .,
        tau = tau,
  ...
  lambda_opt <- optimize(AIC_calc,
                         interval = c(-1, 7),
                         tau = tau,
                         data = df)$min
  
  Y = rqss(pc ~ qss(time, lambda = exp(lambda_opt)), 
           tau = tau, data = df)
  return(Y)
}


# simul_dfr_1 = simul_dfr %>%
#   group_by(sim_id) %>%
#   mutate(P50 = fitted(AQRREG(., 0.5))) %>%
#   mutate(P10 = fitted(AQRREG(., 0.1))) %>%
#   mutate(P90 = fitted(AQRREG(., 0.9)))

##########################################################################-
# Este enfoque resulta muy caro computacionalmente por lo cual no se va a
# aplicar al presente estudio.
##########################################################################-
...
data_pred %>% 
  ggplot(aes(x = time)) +
  geom_line(aes(y = y_1, group = 'id')) +
  geom_line(aes(y = PRED_E), col = 'green') +
  theme_bw() +
