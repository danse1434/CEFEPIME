...
# Reasignar directorio de trabajo
setwd(dir = file.path(getwd(), 'tbs_folder_2', 'lambda_-0.20'))
...
y_1_residuals <-
  ...
  mutate(across(c(prediction_pwRes, pwRes, pwRes_simBlq, prediction_iwRes_mean,
                  iwRes_mean, npde, prediction_npde), 
                ~((.x*-0.2)+1)^(1/-0.2)))
...
data <- read_delim("data_TSFD.csv", delim = ';')
