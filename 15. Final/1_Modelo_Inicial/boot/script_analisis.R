...
G1 <- param_df1 %>%
  ...
  geom_density() + 
  geom_vline(lines, mapping = aes(xintercept = mn), col = 'green2') +
...
#-------------------------------------------------------------------------------#
# Crear un gráfico con las distribuciones para el set de par?metros estimados 
# por modelamiento.
#................................................................................
...
#-------------------------------------------------------------------------------#
# Crear una tabla de par?metros con valores de resumen del análisis de bootstrap
#................................................................................
...
#    b Desviación estándar del valor de cada par?metro
#    c Coeficiente de variación del valor de cada par?metro
#    d Límite inferior IC95% del valor de cada par?metro
#    e Límite superior IC95% del valor de cada par?metro
#................................................................................
...
param_table_gt <- param_table %>%
  ...
  gt::tab_header(gt::md('**Resultados bootstrap no parámétrico**'),
                 gt::md('**Modelo Final con error combinado**')) %>% 
...
#................................................................................
#  1 Convertir lista en un data.frame con identificación de los datos 
#  remuestreados correspondientes.
#  2 Convertir el data frame en un tibble
#  3 Renombrar algunas columnas del df
#................................................................................
...
convergence_df1 <- group_by(convergence_df, ID) %>% 
  filter(pval_log == F) %>% 
  slice(1)
...
# de exploración (N = 1000). Este criterio deber?a darse durante la fase de 
# exploración del algoritmo SAEM. Se eliminan aquellas que no convergen de 
# manera adecuada.
# 
# De manera ideal, si existe convergencia el criterio de convergencia deber?a
...
# Almacenamiento del gráfico
ggsave('boot/figures/7_resultados_bootstrap_limpio.pdf', G1b, 'pdf',
       width = 8, height = 6)
...
# Almacenamiento del gráfico
ggsave('boot/figures/8_bootstrap_dots_limpia.pdf', G3b, 'pdf',
       width = 6, height = 4)
...
popParameters <- read_csv('BASE_MODEL/populationParameters.txt')
...
param_df3_gt <- param_df3 %>%
  ...
  gt::tab_spanner('Resultados Bootstrap (n = 977)', 
  ...
  gt::fmt_missing(1:15, 1:20, missing_text = '-') %>% 
  gt::tab_header(gt::md('**Resultados bootstrap no parámétrico**'),
                 gt::md('**Modelo Final con error combinado**')) %>% 
  gt::tab_options(table.font.size = "smaller",
                  data_row.padding = gt::px(3)) %>% 
  gt::tab_footnote('Obtenido mediante bootstrap no paramétrico', 
                   gt::cells_column_spanners(spanners = "Resultados Bootstrap (n = 977)")) %>% 
  gt::tab_footnote('Obtenido mediante matriz de info. de Fisher (FIM)', 
                   gt::cells_column_labels(columns = vars(se_sa, rse_sa)))


# Almacenamiento de tabla de datos de HTML
gt::gtsave(param_df3_gt, '9_IC_bootstrap.html', 
           glue('{getwd()}/boot/figures/'))
