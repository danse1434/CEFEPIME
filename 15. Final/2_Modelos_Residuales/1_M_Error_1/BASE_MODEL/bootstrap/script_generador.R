...
setwd(file.path(getwd() ,"tbs_folder_2" ,"boot"))

#-------------------------------------------------------------------------------#
# Generación de subdirectorios de trabajos
set.seed(12356)

data <- read_delim(file.path('..', 'lambda_-0.20', 'data_TSFD.csv'), 
                   delim = ";", na = '.')
...
for (i in 1:1000) {
  dir.create(file.path(paste0('B', i)))
  
  write_delim(
    x = table.ls[[i]],
    path = file.path(glue('B{i}'), glue('data_{i}.csv')),
    delim = ';', na = '.'
  )
}

for (i in 1:1000) {
  file.copy(file.path('..', 'lambda_-0.20', 'final_model.properties'),
            file.path(glue('B{i}')))
}

#-------------------------------------------------------------------------------#
# Apertura del archivo de control
fileName <- file.path('..', 'lambda_-0.20', 'final_model.mlxtran')
...
for (i in 1:1000) {
  B <- A1 %>%
    str_replace("(?<=data\\_)TSFD", "i") %>% 
    str_replace("(?<=data\\_)i(?=\\.csv)", paste(i)) 
  
  write_lines(B, file.path(paste0('B', i),
                           'final_model.mlxtran'), sep = '\n')
}
