...
setwd(file.path('F:', 'Documentos', '(Proyecto)_Estudio_PKPD', 'CEFEPIME', 
                '05. Base_Boot', 'BOOT2', 'Bootstrap_analisis'))
...
for (i in 1:1000) {
  param_list[[i]] = read_csv(file.path('..', paste0('B', i), 'BASE',
...
for (i in 1:1000) {
  convergence_list[[i]] = read_csv(file.path('..', paste0('B', i), 'BASE', 
...
# Almacenamiento del gráfico
ggsave(filename = './FIGURAS/trayectorias_Iter_Small.pdf',
       plot = GITER,
       device = 'pdf',
       width = 6, height = 4) #Se cambia de PDF:10x11 a PNG:4x6
...
popParameters <- read_csv(file.path('F:', 'Documentos', '(Proyecto)_Estudio_PKPD', 
                                    'CEFEPIME', '05. Base_Boot', 
                                    'populationParameters.txt'))
