...
##  seguimiento de LRT con restricciones en las covariables y niveles alpha 
##  de F 0.05, y B 0.01.
##  
...
setwd(file.path('F:','Documentos','(Proyecto)_Estudio_PKPD','CEFEPIME', 
                '12. Covariables', 'BASE_MODEL', 'M_SCM_LRT3'))
...
G2 <- data4 %>% 
  ...
  scale_x_continuous(breaks = round(seq(0, max(data4$Iteracion), length.out = 20)), 
                     sec.axis = dup_axis(breaks = NULL, name = NULL)) + 
