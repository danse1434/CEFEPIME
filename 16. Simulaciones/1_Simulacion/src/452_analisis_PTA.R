...
ggsave('452_analisis_100fTmasMIC_1.pdf', G1, 'pdf', 'figures', 1, 6, 7)
...
# Almacenamiento
ggsave('453_analisis_100fTmasMIC_PTA.pdf', G2, 'pdf', 'figures', 1, 6, 7)
...
for (i in 1:6) {
  RESPTA_60a[[i]] <-
    readRDS(paste0('results/', id_SRC[i], '/RES_PTA_60tmasMIC.rds'))
}

# Crear tabla unificada
RESPTA_60b <- map_dfr(.x = RESPTA_60a, .f = ~.x, .id = 'ID')
...
ggsave('465_analisis_60fTmasMIC_1.pdf', G_60_1, 'pdf', 'figures', 1, 6, 7)
...
# Almacenamiento
ggsave('466_analisis_60fTmasMIC_PTA.pdf', G_60_2, 'pdf', 'figures', 1, 6, 7)
...
for (i in 1:6) {
  RESPTA_5[[i]] <- readRDS(paste0('results/', id_SRC[i], '/RES_PTA_50tmas4MIC.rds'))
}

# Crear tabla unificada
RESPTA_5 <- map_dfr(.x = RESPTA_5, .f = ~.x, .id = 'ID')
...
# Almacenamiento
ggsave('454_análisis_50fTmas4MIC_1.pdf', G5b, 'pdf', 'figures', 1, 6, 7)
...
# Almacenamiento
ggsave('455_analisis_50fTmas4MIC_PTA.pdf', G5c, 'pdf', 'figures', 1, 6, 7)
...
FIM_RESPTA_1 %>% 
  gtsave(filename = '456_Tabla_PTA_100%fTMIC.html', path = file.path(getwd(), 'figures'))

FIM_RESPTA_2 %>% 
  gtsave(filename = '457_Tabla_PTA_50%fT4MIC.html', path = file.path(getwd(), 'figures'))

FIM_RESPTA_60 %>% 
  gtsave(filename = '467_Tabla_PTA_60%fTMIC.html', path = file.path(getwd(), 'figures'))
