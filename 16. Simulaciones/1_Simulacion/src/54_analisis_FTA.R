...
for (i in 1:6) {
  RESPTA_60a[[i]] <- readRDS(paste0('results/L00', i, '/RES_PTA_60tmasMIC.rds'))
}

# Crear tabla unificada
RESPTA_60a <- map_dfr(.x = RESPTA_60a, .f = ~.x)
...
for (i in 1:6) {
  RESPTA_2[[i]] <- readRDS(paste0('results/L00', i, '/RES_PTA_50tmas4MIC.rds'))
}

# Crear tabla unificada
RESPTA_2 <- map_dfr(.x = RESPTA_2, .f = ~.x)
...
FIM_RESPTA_1 %>% 
  gtsave(filename = '63_CFR_Indicador_100fTmasMIC.html', path = file.path(getwd(), 'figures'))

FIM_RESPTA_2 %>% 
  gtsave(filename = '64_CFR_Indicador_50fTmas4MIC.html', path = file.path(getwd(), 'figures'))

FIM_RESPTA_60 %>% 
  gtsave(filename = '68_CFR_Indicador_60fTmasMIC.html', path = file.path(getwd(), 'figures'))
