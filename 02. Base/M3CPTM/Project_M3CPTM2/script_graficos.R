...
setwd(file.path('F:','Documentos','(Proyecto)_Estudio_PKPD','CEFEPIME',
                '02. Base','M3CPTM','Project_M3CPTM2'))
...
GPRED_TOTAL <- G_PRED_OBS_IPRED + G_PRED_OBS_IPREDLOG +
  G_PRED_OBS_PRED + G_PRED_OBS_PREDLOG + plot_annotation(tag_levels = 'A')
...
G_RES_TAD_PWRES <- 
  RES_TAD(x = TAD, y = pwRes, xlab = 'TAD (h)', ylab = 'WRES')
...
eta_shrink_V1 <- 1 - (var(eta$eta_V1_mean) / (0.285 ^ 2))
eta_shrink_V2 <- 1 - (var(eta$eta_V2_mean) / (0.807 ^ 2))
eta_shrink_V3 <- 1 - (var(eta$eta_V3_mean) / ((5.4e-8) ^ 2))

eta_shrink_Cl <- 1 - (var(eta$eta_Cl_mean) / (0.261 ^ 2))
eta_shrink_Q2 <- 1 - (var(eta$eta_Q2_mean) / (0.916 ^ 2))
eta_shrink_Q3 <- 1 - (var(eta$eta_Q3_mean) / ((5.35e-8) ^ 2))
