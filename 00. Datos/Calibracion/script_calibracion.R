...
out_det = Vectorize(FUN=out_det, vectorize.args = 'val') # Vectorizar función
  ...
  ggsave(filename = './Resultados/Figuras/01_Outliers_identificados.pdf', plot = g1_plot, 
...
# Análisis de Exactitud ---------------------------------------------------
  ...
  D3_COUT %>% 
    ...
    stat_smooth(data=D3_COUT, aes(x=log_conc, y=m_Diam),
                method='lm',inherit.aes = FALSE) +
    xlab('log(concentración)') + ylab('Diámetro de halo')
  ggsave(filename = './Resultados/Figuras/02_curva_calibracion_1.pdf', plot = g2_plot, 
         device = 'pdf', width = 5.13*1.5, height = 4.29)
  #####################################################################################################-
  ...
  colnames(reg_sum) = rep(c('Estimate', 'Std. Error', 't value', 'Pr(>|t|)'),2)
  ...
  # modelo escogido N.4.
  g3_plot = 
  ggplot(data=filter(L1.df, Modelo==4), aes(x = m_Conc, y = Pred., 
  ...
  ggsave(filename = './Resultados/Figuras/03_Comparacion_CC_1.pdf', plot = g3_plot, 
  ...
  ggplot(data=filter(L1.df, Modelo==4), 
    ...
    theme(panel.grid = element_line(colour = 'gray98'))
  ggsave(filename = './Resultados/Figuras/05_Grafico_error_concentracion.pdf', plot = g5_plot, 
         device = 'pdf', width = 5.13*1.2, height = 4.29)  
  ...
  ggsave(filename = './Resultados/Figuras/06_Grafico_error_diametro_halo.pdf', plot=g6_plot, 
         device = 'pdf', width = 5.13*1.2, height = 4.29)  
  ...
  L1.df %>% 
    ...
    theme(panel.grid = element_line(colour = 'gray98'))
  ggsave(filename = './Resultados/Figuras/07_Exactitud_metodo.pdf', plot=g7_plot, 
         device = 'pdf', width = 5.13, height = 4.29)    
  ...
  L1.df %>% 
    ...
    ylab('RSD')  +
    theme(panel.grid = element_line(colour = 'gray98'))
  ggsave(filename = './Resultados/Figuras/08_Repetibilidad_método.pdf', plot=g8_plot, 
         device = 'pdf', width = 5.13, height = 4.29)  
  #####################################################################################################-
  # Límite de Cuantificación
  LOQ = loq(object = list_reg[[4]], alpha=0.05, n = 6, w.loq=log(3.125))
  
  # LOD = lod(object = list_reg[[4]], alpha=0.05)
